...
@section('title', 'ELM GROVE LIQUOR | Premium Spirits')

@section('content')
<!-- Hero Section -->
<div class="relative h-screen flex items-center justify-center overflow-hidden">
    <div class="absolute inset-0 z-0">
        <img src="{{ asset('css/image.jpg') }}" alt="Premium Liquor Background" class="w-full h-full object-cover opacity-40">
    ...
    <div class="container mx-auto px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @foreach($categories as $category)
                <a href="{{ route('shop.category', $category->slug) }}" class="group relative h-96 overflow-hidden rounded-sm glass-card hover:border-gold-500/50 transition-all duration-500">
                    @if($category->image)
                        <img src="{{ Storage::url($category->image) }}" alt="{{ $category->name }}" class="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-110 transition-transform duration-700">
                    @else
                        <img src="https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?q=80&w=2670&auto=format&fit=crop" alt="{{ $category->name }}" class="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-110 transition-transform duration-700">
                    @endif
                    <div class="absolute inset-0 bg-gradient-to-t from-black-950 via-transparent to-transparent opacity-90"></div>
                    <div class="absolute bottom-0 left-0 p-8">
                        <h3 class="text-2xl font-heading text-white mb-2 group-hover:text-gold-400 transition-colors">{{ $category->name }}</h3>
                        <p class="text-gray-400 text-sm">{{ $category->description ?? 'Explore Collection' }}</p>
                    </div>
                </a>
            @endforeach
        </div>
