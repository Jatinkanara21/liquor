...
@section('title', 'Add New Category | Elite Liquor House')
...
<div class="bg-black-900 min-h-screen py-10">
    <div class="container mx-auto px-6">
        <div class="flex justify-between items-center mb-10 border-b border-white/10 pb-6">
            <h1 class="text-3xl font-heading text-white">Add <span class="text-gold-500">New Category</span></h1>
        ...
        <div class="max-w-2xl mx-auto">
            <form action="{{ route('admin.categories.store') }}" method="POST" class="glass p-8 rounded-sm" enctype="multipart/form-data">
                @csrf
                
                <div class="mb-8">
                    <label for="name" class="block text-gray-400 text-sm mb-2 uppercase tracking-wide">Category Name</label>
                    <input type="text" name="name" id="name" class="w-full bg-black-950 border border-white/10 text-white px-4 py-3 rounded-sm focus:border-gold-500 focus:outline-none focus:ring-1 focus:ring-gold-500/20" value="{{ old('name') }}" required autofocus>
                    <p class="text-gray-500 text-xs mt-2">The slug will be automatically generated from the name.</p>
                </div>

                <div class="mb-8">
                    <label for="description" class="block text-gray-400 text-sm mb-2 uppercase tracking-wide">Description</label>
                    <textarea name="description" id="description" rows="4" class="w-full bg-black-950 border border-white/10 text-white px-4 py-3 rounded-sm focus:border-gold-500 focus:outline-none focus:ring-1 focus:ring-gold-500/20">{{ old('description') }}</textarea>
                </div>
